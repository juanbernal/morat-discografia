...
const ContactForm: React.FC<{ albums?: Album[], tracks?: Track[] }> = ({ albums = [], tracks = [] }) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const body = `Nombre: ${data.name}%0D%0AEmail: ${data.email}%0D%0AMensaje: ${data.message}`;
            window.location.href = `mailto:contacto@diosmasgym.com?subject=${encodeURIComponent(data._subject as string)}&body=${body}`;
